...
public class PatientServiceImpl implements PatientService {
     ...
     */
    @Override
    public HashMap<String, Object> findPatients(int pageNumber, int size, String query) {
     ...
     */
    @Override
    public HashMap<String, Object> findVPatients(int pageNumber, int size, String query) {
        Page<Patient> page = new Page<>(pageNumber, size);
        QueryWrapper<Patient> wrapper = new QueryWrapper<>();
        wrapper.like("p_name", query)
                .eq("p_isvip", 1)
    ...
    @Override
    public Object topUp(int pId, int balance) {
        QueryWrapper<Patient> wrapper = new QueryWrapper<>();
        wrapper.eq("p_id", pId);
        Patient patient = this.patientMapper.selectOne(wrapper);
        int pBalance = patient.getPBalance();
        int pIntegral = patient.getPIntegral();
        pIntegral +=balance/10;
        pBalance +=balance;
        this.patientMapper.updatePatient(pId, pIntegral, pBalance);
        return true;
    }

    @Override
    public Object findPrice(int dId) {
        Map<String,Object> map = new HashMap<>();
        int price = this.patientMapper.selectPrice(dId);
        map.put("dPrice", price);
        return map;
    }
