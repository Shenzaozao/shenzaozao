...
public class OrderServiceImpl implements OrderService {
    ...
    @Override
    public Object updateBalance(int dId, int pId, int oTotalPrice) {
        Map<String, Object> map = new HashMap<>();
        int p_isExist = this.orderMapper.selectPrize(pId);
        int p_amount = 0;
        map.put("other", 0);
        map.put("prize", 0);
        //用兑换券抵扣
        if (p_isExist == 1){
            p_amount = this.orderMapper.selectDoctorPrize(dId);
            this.orderMapper.updatePrize(pId);
            oTotalPrice = oTotalPrice - p_amount;
            map.put("prize", p_amount);
        }
        //用余额支付
        int p_balance = this.orderMapper.selectBalance(pId);
        if (p_balance >= oTotalPrice){
            p_balance = p_balance - oTotalPrice;
            this.orderMapper.updateBalance(pId, p_balance);
            map.put("balance" ,oTotalPrice);
        }else {
            oTotalPrice = oTotalPrice - p_balance;
            this.orderMapper.updateBalance(pId, 0);
            map.put("balance" ,p_balance);
            map.put("other", oTotalPrice);
        }
        return map;
    }
}
